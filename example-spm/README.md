This is the React Native 0.87 SwiftPM integration example for the local
`@preeternal/react-native-cookie-manager` workspace package. Its iOS app does
not use CocoaPods.

The app resolves the library from the monorepo through React Native autolinking
and Metro, without a `file:..` dependency.
...
Open `ios/CookieManagerExampleSpm.xcodeproj` in Xcode when you want to run it on
